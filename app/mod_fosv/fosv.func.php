...
function fosv_callback(){
	...
	if(d()->validate('callback')){
		...
		$emails = explode(',', d()->Option->email_send);
		//$emails_callback = explode(',', d()->Option->email_callback);
		
		//$all_emails = array_merge($emails, $emails_callback);
		...
		foreach ($emails as $email) {
			...
			$headers = 'From: no-reply.metropol@mail.ru' . "\r\n";
